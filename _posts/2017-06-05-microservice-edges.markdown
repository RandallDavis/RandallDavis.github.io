...
* Team Client needs to visibility into the depth of their testing coverage of the client's interactions with the host's edge.
...
So here's what I think we need to do. The host's API needs to ship with an intelligent fake that Team Host maintains. The client's unit tests should be able to leverage the host's fake in isolation to test real-world behavior. The fake is still only going to be as good as Team Host conceives of being necessary (although I have some ideas to help with this), but that's already a weakness in any unit testing. What we've gained here is that the host's fake evolves alongside the host's behavior and is owned by Team Host. It's more likely to be true and complete. There are other benefits and considerations, but this is the big win.
...
If the host rolls out a new behavior, and the client is a couple weeks behind in adopting it, it's a huge pain point if there's any incompatability. Either the client has to catch up under duress or the host has to figure out how to roll back (which might cause problems for other microservices that are already relying on the new behavior). A simple lack of coordination just caused a real problem. Discipline and transparency in upcoming behavior changes is a huge help in heading these situations off.
...
However, we want to keep our contracts simple, and errors are a part of that contract. The error information that the host provides to the client should have a decent amount of abstraction. Should the client be aware of which part of the infrastructure that's giving the host trouble? Probably not. In my opinion, errors should be sculpted to the needs of the client's perspective - usually that results in things being very simple. Since Team Client is getting a simple contract for error conditions, there should be no barrier in incorporating handling logic and writing unit tests to defend that logic. We all just got happier.
...
The real trick here is that the client will need a way to ask the host's fake to express one of these behaviors. Since that part of the client's communication to the host's edge would never exist in Production, it's not going to fit naturally into the host's contract. That's okay. Let's just know that up front and keep things organized. The fake's contract is going to end up being an extension of the host's contract, and we'll need to explicitly communicate which contract components are exclusively for testing scenarios.
...
I propose that intelligent fakes should be shipped along with microserve contracts to simulate microservice edge behavior, and that the proper practices be put in place to ensure those fakes' vadility. The glue between microservices interactions is the flimsiest part of microservice architectures, and this directly addresses the ambiguities that currently seem to be prevalent.
